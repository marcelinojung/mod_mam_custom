...
access_preferences(Host) ->
    gen_mod:get_module_opt(Host, mod_mam_custom, access_preferences).
...
assume_mam_usage(Host) ->
    gen_mod:get_module_opt(Host, mod_mam_custom, assume_mam_usage).
...
cache_size(Host) ->
    gen_mod:get_module_opt(Host, mod_mam_custom, cache_size).
...
request_activates_archiving(Host) ->
    gen_mod:get_module_opt(Host, mod_mam_custom, request_activates_archiving).

-spec use_cache(gen_mod:opts() | global | binary()) -> boolean().
use_cache(Opts) when is_map(Opts) ->
    gen_mod:get_opt(use_cache, Opts);
use_cache(Host) ->
    gen_mod:get_module_opt(Host, mod_mam_custom, use_cache).
...
user_mucsub_from_muc_archive(Host) ->
    gen_mod:get_module_opt(Host, mod_mam_custom, user_mucsub_from_muc_archive).
