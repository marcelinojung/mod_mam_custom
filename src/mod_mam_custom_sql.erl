...
-module(mod_mam_custom_sql).


-behaviour(mod_mam_custom).
...
store(Pkt, LServer, {LUser, LHost}, Type, Peer, Nick, _Dir, TS) ->
    ...
    XML = case mod_mam_custom_opt:compress_xml(LServer) of
	      true ->
		  J1 = case Type of
		...
		fun([TS, XML, PeerBin, Kind, Nick]) ->
		   ...
		   ([User, TS, XML, PeerBin, Kind, Nick]) ->
		       case make_archive_el(User, TS, XML, PeerBin, Kind, Nick,
					    ...
					    jid:decode(User)) of
			   {ok, El} ->
			       mod_mam_custom:wrap_as_mucsub([{TS, binary_to_integer(TS), El}],
						      JidRequestor);
			   {error, _} ->
			       []
		       end
