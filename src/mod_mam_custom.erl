...
%%%===================================================================
start(Host, Opts) ->
    case mod_mam_custom_opt:db_type(Opts) of
	mnesia ->
	...
	ok ->
	    ...
	    case mod_mam_custom_opt:assume_mam_usage(Opts) of
		true ->
	    ...
	    case mod_mam_custom_opt:clear_archive_on_room_destroy(Opts) of
		true ->
...
use_cache(Mod, Host) ->
	...
	false -> mod_mam_custom_opt:use_cache(Host)
    end.
...
cache_opts(Opts) ->
    MaxSize = mod_mam_custom_opt:cache_size(Opts),
    CacheMissed = mod_mam_custom_opt:cache_missed(Opts),
    LifeTime = mod_mam_custom_opt:cache_life_time(Opts),
...
stop(Host) ->
    ...
    ejabberd_hooks:delete(store_mam_message, Host, ?MODULE,
			  store_mam_message, 100),
    case mod_mam_custom_opt:assume_mam_usage(Host) of
	true ->
    ...
    case mod_mam_custom_opt:clear_archive_on_room_destroy(Host) of
	true ->
...
reload(Host, NewOpts, OldOpts) ->
    ...
    case {mod_mam_custom_opt:assume_mam_usage(NewOpts),
	  mod_mam_custom_opt:assume_mam_usage(OldOpts)} of
	{true, false} ->
	    ejabberd_hooks:add(message_is_archived, Host, ?MODULE,
			       message_is_archived, 50);
	{false, true} ->
...
message_is_archived(false, #{lserver := LServer}, Pkt) ->
    case mod_mam_custom_opt:assume_mam_usage(LServer) of
	true ->
...
delete_old_messages(TypeBin, Days) when TypeBin == <<"chat">>;
    ...
    DBTypes = lists:usort(
		lists:map(
		  fun(Host) ->
			  case mod_mam_custom_opt:db_type(Host) of
			      sql -> {sql, Host};
			      Other -> {Other, global}
			  end
...
process_iq(#iq{from = #jid{luser = LUser, lserver = LServer},
	       ...
	       sub_els = [#mam_prefs{xmlns = NS,
				     ...
				     always = Always0,
				     never = Never0}]} = IQ) ->
    Access = mod_mam_custom_opt:access_preferences(LServer),
...
store_msg(Pkt, LUser, LServer, Peer, Dir) ->
    case get_prefs(LUser, LServer) of
	{ok, Prefs} ->
	    UseMucArchive = mod_mam_custom_opt:user_mucsub_from_muc_archive(LServer),
...
get_prefs(LUser, LServer) ->
	...
	{error, _} ->
	    {error, db_failure};
	error ->
	    ActivateOpt = mod_mam_custom_opt:request_activates_archiving(LServer),
	    case ActivateOpt of
		true ->
		    {ok, #archive_prefs{us = {LUser, LServer}, default = never}};
		false ->
		    Default = mod_mam_custom_opt:default(LServer),
...
select(LServer, JidRequestor, JidArchive, Query, RSM, MsgType, Flags) ->
	...
	true ->
	    {[], true, 0};
	false ->
	    case {MsgType, mod_mam_custom_opt:user_mucsub_from_muc_archive(LServer)} of
		{chat, true} ->
